...
**Recommendation in one sentence:** JaBot should treat **Code as ACP harness
sessions** and every other crew bot as a **thin host-owned LLM + MCP
allowlist** (prompt, tools, color, per-bot memory files) — Chief routes by
**opening or spawning a worker thread**, not by nesting Claude Code subagents
or adopting CrewAI / LangGraph.

| Question | Short answer | Detail |
|---|---|---|
| 1. What is a bot? | Two kinds. Code = ACP harness session. Everyone else = system prompt + MCP allowlist on a host-owned model loop. | [bot-model.md](bot-model.md) |
...
| 4. Templates | Shipped JSON: name, color, instructions, tool ids. Core crew is pre-installed; Expense / Talent / Social / Ops are add-from-template snapshots. | [templates-memory-schedules.md](templates-memory-schedules.md#templates) |
...
| 6. Schedules | In-process cron in the JaBot host. A job is `{botId, cron, prompt}`. Fire = run that bot, fold to Inbox. Not launchd, not EventKit. | [templates-memory-schedules.md](templates-memory-schedules.md#schedules) |
...
2. **Chief of Staff chat** — thin model loop + host tools
   (`handoff_to_bot`, `spawn_code_session`, `list_crew_status`). Not an ACP
   coding session. See [routing.md](routing.md).
3. **Tool/MCP connection framework** — one host-owned MCP catalog; OAuth in
   ...
   `session/new` when a **code** thread needs them. See
...
Do **not** run Inbox Mgr or Writer inside Claude Code just because the
Agent SDK can spawn subagents. Subagents live *inside one coding session*
and return a summary; they are the wrong grain for messenger bots.
