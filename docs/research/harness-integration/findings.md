...
|---|---|---|
| 1. Integration mode | Headless + SDK + ACP adapters exist for Claude Code, Codex, Pi, Gemini CLI, and Cursor (`agent acp`). No TUI scraping. Aider is a later preset: scripting CLI behind a JaBot-owned ACP wrapper, not native ACP. | [claude-code.md](claude-code.md), [codex.md](codex.md), [pi.md](pi.md), [gemini.md](gemini.md), [aider.md](aider.md), [cursor.md](cursor.md) |
