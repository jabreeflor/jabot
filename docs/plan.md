...
build work. The umbrella with the full dependency graph is
[#30](https://github.com/jabreeflor/jabot/issues/30).

## Decisions (settle first — they gate the rest)

| Issue | Decision | Gates |
|---|---|---|
| [#4](https://github.com/jabreeflor/jabot/issues/4) | Physical host process & quit policy (in-process Tauri host vs durable daemon) | scaffold, host API, supervisor |
| [#5](https://github.com/jabreeflor/jabot/issues/5) | Fold/run/Inbox data model (thread-state overlay vs three stores) | schema, state machine, Inbox |
| [#6](https://github.com/jabreeflor/jabot/issues/6) | What is a bot? (prompt + MCP allowlist vs isolated scope) | crew store, Chief |
...
| [#13](https://github.com/jabreeflor/jabot/issues/13) | Harness catalog + Doctor (Claude/Codex/Pi, Hermes/OpenClaw, custom JSON) | #10 |
...
| [#17](https://github.com/jabreeflor/jabot/issues/17) | Crew store + CRUD + bot templates as data | #6, #9, #11 |
...
| [#24](https://github.com/jabreeflor/jabot/issues/24) | Chief of Staff bot: host-owned loop + handoff/spawn/status tools | #17, #18 |
