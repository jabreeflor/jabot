...
| **secret round-trip** | `put` then `get` of a throwaway secret via the **OS** store | Host APIs land with [#283](https://github.com/jabreeflor/jabot/issues/283). Live cell still needs a Windows box (or #286 runner); Linux `windows-secrets-check.sh` is fail-closed only | Credential Manager item under an isolated service, not `JABOT_SECRETS_BACKEND=memory` |
