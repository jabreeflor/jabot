...
| `src-tauri/src/host/store/secrets.rs` (`os_put` / `os_get` / `os_delete`) | macOS Keychain via `keyring` `apple-native`; Windows Credential Manager via `windows-native` (#283) | **macos clippy** (native) for the Apple path | `keyring` / Security.framework are not in the notify scratch crate. Windows compile of the same file is `scripts/windows-secrets-check.sh` / a later Windows runner (#286). |
