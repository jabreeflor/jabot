...
The workspace sources are now the only selection controls: there is no second folder dropdown. `selected-workspace.png` shows the chosen folder's name and path with a Remove action; removing it returns the modal to the empty workspace state and hides Advanced controls.
