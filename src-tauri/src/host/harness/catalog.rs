...
pub enum Readiness {
    ...
    /// Gather facts and classify them. Copilot's version, login, policy, and
    /// model cannot be reduced to one exit code.
    Inspect,
}
...
enum CompiledReadiness {
    ...
    /// Facts-then-classify, used when an exit code is not the diagnosis.
    /// Copilot's CLI *is* the ACP adapter; version, login, policy, and model
    /// are separate sentences (`copilot.rs`).
    Inspect,
}
...
const SHIPPED: &[Compiled] = &[
    ...
    Compiled {
        ...
        readiness: CompiledReadiness::Inspect,
        session_scope: SessionScope::Thread,
    },
];
...
fn build(compiled: &Compiled) -> HarnessDescriptor {
    HarnessDescriptor {
        ...
        readiness: match &compiled.readiness {
            ...
            CompiledReadiness::Inspect => Readiness::Inspect,
        },
...
mod tests {
    ...
    fn shipped_ids_are_the_reserved_cards() {
        ...
        assert_eq!(ids, ["claude", "codex", "pi", "copilot"]);
        for id in ids {
    ...
    fn copilot_is_a_shipped_card_that_launches_acp_and_does_not_claim_resume() {
        ...
        assert!(matches!(copilot.readiness, Readiness::Inspect));
        let caps = copilot
            .capabilities
            .as_ref()
