...
use super::protocol::methods::{
    ...
    ThreadSetModelParams, ThreadSourceAddParams, ThreadSourceRefParams, ThreadTranscriptParams,
    ToolRefParams, CREW_CREATE, CREW_DRAFTS, CREW_DRAFT_DISMISS, CREW_DRAFT_GET, CREW_DRAFT_SAVE,
    CREW_LIST, CREW_REMOVE, CREW_THREAD, CREW_UPDATE, FOLDER_FORGET, FOLDER_LIST, FOLDER_REGISTER,
    ...
    THREAD_RESUME, THREAD_SET_MODEL, THREAD_SOURCE_ADD, THREAD_SOURCE_OPEN, THREAD_SOURCE_REMOVE,
    THREAD_STATE, THREAD_SUMMARY, THREAD_TRANSCRIPT, TOOLS_CONNECT, TOOLS_DISCONNECT, TOOLS_LIST,
};
use super::protocol::methods::{
...
fn handle(session: &mut HostSession, request: &JsonRpcRequest) -> Result<Value, RpcError> {
    ...
    match request.method.as_str() {
        ...
        THREAD_SET_MODEL => {
            session.require_hello()?;
            let params: ThreadSetModelParams = parse_params(request.params.as_ref())?;
            params.validate()?;
            to_value(session.thread_set_model(params)?)
        }
        THREAD_RESUME => {
            session.require_hello()?;
            let params: ThreadRefParams = parse_params(request.params.as_ref())?;
