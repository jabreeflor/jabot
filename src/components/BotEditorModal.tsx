        ...
        <div className="bot-icon-grid">
          {BOT_ICONS.map((choice) => (
            <label className="bot-icon-option" key={choice.value}>
              <input type="radio" name={iconGroupId} value={choice.value}
                aria-label={choice.name} checked={!image && color === choice.value}
                onChange={() => { setColor(choice.value); setImage(null); setImageError(null); }} />
