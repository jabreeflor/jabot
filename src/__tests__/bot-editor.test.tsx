...
describe("the animated bot picker", () => {
  ...
  it("replaces an uploaded picture when the user selects a built-in bot", async () => {
    const props = renderEditor({
      bot: { ...WRITER, image: "data:image/webp;base64,AAAA" },
    });
    ...
    expect(document.querySelector(".iconpick .bot-mark")).toHaveAttribute(
      "data-character",
      "classic",
    );
    await userEvent.click(screen.getByRole("button", { name: "Save" }));
    expect(props.onSave).toHaveBeenCalledWith(
      expect.objectContaining({ color: "b-teal", image: null }),
    );
  });
});
