...
export function cargoDebugDir(): string {
  const hostd =
    process.platform === "win32" ? "jabot-hostd.exe" : "jabot-hostd";
  const fake =
    process.platform === "win32" ? "fake-acp-agent.exe" : "fake-acp-agent";
...
function cargoDebugCandidates(): string[] {
  ...
  dirs.push(
    path.join(repoRoot, "src-tauri", "target", "llvm-cov-target", "debug"),
  );
  return dirs;
}
