...
test.describe("PR workspace", () => {
  test("sign-in refusal, board search/filter, details, reopen, failed refresh", async ({
    ...
    const folder = await jabot.rpc<{ folderId: string; cwd: string }>(
      "folder/register",
      {
        path: repo.dir,
        name: "jabot",
    ...
    let listed = {
      pullRequests: [] as Array<{ number: number; title: string }>,
    };
    ...
    const filled = await jabot.rpc<{ pullRequests: Array<{ title: string }> }>(
      "pr/list",
    );
    expect(
      filled.pullRequests.some((pr) => /Migrate auth/i.test(pr.title)),
    ).toBe(true);
    ...
    await expect(
      page.getByRole("status").filter({ hasText: /fail|unavailable|refresh/i }),
    ).toBeVisible();
