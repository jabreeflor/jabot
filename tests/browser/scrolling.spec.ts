...
test.describe("scrolling", () => {
  test("follows the tail, holds when scrolled up, and jumps back", async ({
    ...
    await expect(agentBubble(page).filter({ hasText: "streamed output" })).toBeVisible();

    await expect
      .poll(async () => scroller.evaluate((el) => el.scrollHeight - el.clientHeight > 40))
      .toBe(true);

    await expect
      .poll(async () =>
        scroller.evaluate((el) => el.scrollHeight - el.scrollTop - el.clientHeight <= 32),
      )
      .toBe(true);
    ...
    await expect(agentBubble(page).filter({ hasText: "second-wave" })).toBeVisible();

    await expect.poll(async () => scroller.evaluate((el) => el.scrollTop)).toBe(held);
    ...
    await expect(userBubble(page).filter({ hasText: "back to the tail" })).toBeVisible();
    await expect
      .poll(async () =>
        scroller.evaluate((el) => el.scrollHeight - el.scrollTop - el.clientHeight <= 32),
      )
